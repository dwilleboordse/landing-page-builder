...
${brief.objections || "Not specified"}`;
  ...
  if (brief.uploadedContent) {
    prompt += `\n\nUPLOADED DOCUMENT CONTENT:\n${brief.uploadedContent}`;
  }

  if (brief.pageType === "listicle") {
    prompt += `\n\nLISTICLE SPECIFICS:\nNumber of items: ${brief.listicleCount || 5}\nProduct position in list: ${brief.listiclePosition || "#1"}`;
  }

  if (brief.pageType === "comparison") {
    prompt += `\n\nCOMPARISON SPECIFICS:\nCompare against: ${brief.compareAgainst || "Generic alternatives"}`;
  }
